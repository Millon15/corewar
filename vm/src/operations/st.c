...
/*   Updated: 2018/10/10 20:42:15 by akupriia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline int	calc_fa(int tmp, int space_t_end)
{
	...
	if (tmp > IDX_MOD && !(tmp % IDX_MOD))
		first_arg = 0;
	// else if (tmp % IDX_MOD != tmp % MEM_SIZE && tmp > MEM_SIZE * 2 && tmp < FPOS)
	// 	first_arg = tmp % IDX_MOD;
	// else if (tmp == 52483)
	// 	first_arg = tmp % IDX_MOD - IDX_MOD;	
	// else if (tmp == 6659)
	// 	first_arg = (tmp > IDX_MOD) ?
	// 	tmp % IDX_MOD : tmp;
	else if ((ft_abs(tmp - SHORT_RANGE) <= IDX_MOD) ||
	(ft_abs(tmp - SHORT_RANGE) <= IDX_MOD) || (tmp > SHORT_RANGE / 2 && tmp < SHORT_RANGE - MEM_SIZE * 2
	&& (tmp - SHORT_RANGE) % MEM_SIZE == tmp % IDX_MOD - IDX_MOD)) // || ((tmp > SHORT_RANGE / 2) && (tmp < SHORT_RANGE - MEM_SIZE * 2) && (tmp % IDX_MOD == tmp % MEM_SIZE)) 
	...
	|| (tmp % SHORT_RANGE >= MEM_SIZE * 2 && tmp % SHORT_RANGE <= SHORT_RANGE / 2) || (tmp > MEM_SIZE && tmp <= MEM_SIZE * 2/* && (tmp % MEM_SIZE >= MEM_SIZE / 2)*/) || ((ft_abs(tmp - SHORT_RANGE) <= MEM_SIZE * 2)
	&& (tmp % IDX_MOD == tmp % MEM_SIZE)) || (tmp > SHORT_RANGE / 2 && tmp < SHORT_RANGE - MEM_SIZE * 2)/* || (tmp == 16962)*/)
	{
		// first_arg = (tmp > MEM_SIZE) ?
		// tmp % IDX_MOD - IDX_MOD : tmp % IDX_MOD;
		first_arg = (tmp > IDX_MOD) ?
		tmp % IDX_MOD : tmp;
		if (tmp > MEM_SIZE && !(tmp % SHORT_RANGE >= MEM_SIZE * 2 && tmp % SHORT_RANGE <= SHORT_RANGE / 2) && !(tmp > MEM_SIZE && tmp <= MEM_SIZE * 2)/* && (tmp != 16962)*/)
			first_arg -= IDX_MOD;
	}
	else if ((/*(tmp > MEM_SIZE && tmp < MEM_SIZE * 2)
	|| */(tmp % IDX_MOD == tmp % MEM_SIZE)
	|| (tmp > MEM_SIZE * 2 && tmp < FPOS) ||
	((tmp > IDX_MOD) && (tmp <= MEM_SIZE) && !(tmp % IDX_MOD)) ||
	((tmp > MEM_SIZE * 2) && !(tmp % IDX_MOD) && ((tmp % MEM_SIZE) <= MEM_SIZE / 2)
	&& (!((tmp % MEM_SIZE) % IDX_MOD))))/* && ((tmp - SHORT_RANGE) % MEM_SIZE == tmp % IDX_MOD - IDX_MOD)*/)
...
static inline void	alter_for_verbose(int tmp, int *first_arg)
{
	...
	|| (tmp > SHORT_RANGE / 2 && tmp < SHORT_RANGE - MEM_SIZE * 2))
		*first_arg = tmp - SHORT_RANGE;
